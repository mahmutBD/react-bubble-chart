  ...
  _createClass(ReactBubbleChartD3, [{
    ...
    value: function update(el, props) {
      ...
      var color = function color(index) {
        return _this2.colorRange[index];
      };
      ...
      labels.html(function (d) {
        return d.value;
      }).on('mouseover', this._tooltipMouseOver.bind(this, color, el)).transition().duration(duration).delay(function (d, i) {
        return i * delay;
        ...
        return d.y - d.r + 'px';
      }).style('opacity', 1).style('z-index', function (d, i) {
        return labels[0].length - i;
